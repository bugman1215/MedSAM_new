...
nii_path = "data/flare/images"  # path to the nii images
gt_path = "data/flare/labels"  # path to the ground truth
npy_path = "data/npy/flare" + prefix[:-1]
...
def save_as_jpeg(image_array, path, quality=80):
    """
    将图像保存为 JPEG 格式
    :param image_array: 需要保存的图像 NumPy 数组
    :param path: 保存路径
    :param quality: JPEG 压缩质量 (0-100)
    """
    img = Image.fromarray(np.uint8(image_array))  # 将 NumPy 数组转换为 PIL 图像
    img.save(path, format='JPEG', quality=quality)  # 保存为 JPEG 格式，设置压缩质量

# Function to save label as PNG
def save_as_png(label_array, path):
    """
    将标签保存为 PNG 格式
    :param label_array: 需要保存的标签 NumPy 数组
    :param path: 保存路径
    """
    img = Image.fromarray(np.uint8(label_array))  # 将 NumPy 数组转换为 PIL 图像
    img.save(path, format='PNG')  # 保存为 PNG 格式

# %% save preprocessed images and masks as JPEG and PNG files
for organ_label in organ_labels:
    ...
    for name in tqdm(names):  # 遍历所有病例
        ...
        if len(z_index) > 0:
            ...
            # Save each slice of the image as JPEG file
            for i in range(img_roi.shape[0]):
                ...
                save_as_jpeg(resize_img_skimg_01 * 255.0, jpeg_save_path, quality=80)

            # 保存标签（ground truth）为 PNG 文件
            for i in range(gt_roi.shape[0]):
                gt_i = gt_roi[i, :, :]
                ...
                save_as_png(resize_gt_skimg, png_save_path)
